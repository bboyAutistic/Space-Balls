...
public class PlayerInputMP : NetworkBehaviour {
	...
	void Awake(){
		...
		laser = GetComponentsInChildren<LaserMP> ();
		target = null;
		...
		if (Input.GetKey(KeyCode.Mouse0))
		{
			...
			for (int i = 0; i < laser.Length; i++) {
				if (laser [i].getCanFire()) {
					CmdFireLaser (laser [i].transform.position, laser [i].transform.forward, laser [i].maxDistance, laser [i].damage, i);
					laser [i].ReloadLaser ();
				}
			}
	...
	[Command]
	void CmdFireLaser(Vector3 origin, Vector3 direction, float maxDistance, float damage, int laserIndex){
		RaycastHit hit;
		if (Physics.Raycast (origin, direction, out hit, maxDistance)) {
			...
			if (hit.collider.gameObject.CompareTag ("Player")) {
				...
				PlayerHealthMP player = hit.collider.gameObject.GetComponent<PlayerHealthMP> ();
			...
			RpcShowLaser (hit.point, laserIndex);

		} else {
			RpcShowLaser (origin + (direction * maxDistance), laserIndex);
		}
	}

	[ClientRpc]
	void RpcShowLaser(Vector3 hitPoint, int laserIndex){
		laser [laserIndex].LaserEffectsOnly(hitPoint);
	}
}